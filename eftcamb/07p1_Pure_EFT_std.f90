...
module EFTCAMB_pure_EFT_std
    ...
    type, extends ( EFTCAMB_designer_model ) :: EFTCAMB_std_pure_EFT
        ...
        !EFT fuctions coupling Q model
        integer  :: EFTCQ              !< Model selection flag for Pure EFT Q.
        ...
        ! the pure EFT functions for Q:
        class( parametrized_function_1D ), allocatable :: PureEFTCQ      !< The pure EFT function Q.
...
contains
    ...
    subroutine EFTCAMBPureEFTstdReadModelSelectionFromFile( self, Ini )
        ...
        ! read the Coupled Q flag:
        self%EFTCQ  = Ini_Read_Int_File( Ini, 'EFTCQ'  , 0 )!
    end subroutine EFTCAMBPureEFTstdReadModelSelectionFromFile

    ! ---------------------------------Ini_Read_Int_File( Ini, 'EFTCQ'  , 0 ------------------------------------------------------------
    ...
    subroutine EFTCAMBPureEFTstdAllocateModelSelection( self, Ini )
        ...
        ! allocate Q:!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if ( allocated(self%PureEFTCQ) ) deallocate(self%PureEFTCQ)
        select case ( self%EFTCQ )
            case(0)
                allocate( zero_parametrization_1D::self%PureEFTCQ)
            case(1)
                allocate( constant_parametrization_1D::self%PureEFTCQ )
                !call self%PureETCQ%set_param_names()
            case default
                write(*,'(a,I3)') 'No model corresponding to EFTCQ =', self%EFTCQ
                write(*,'(a)')    'Please select an appropriate model.'
        end select
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        call self%PureEFTCQ%set_name   ( 'EFTq'     , 'q'            )! initialize the Q names:
        ...
        call self%PureEFTCQ%init_func_from_file   ( Ini )
    ...
    subroutine EFTCAMBPureEFTstdInitModelParameters( self, array )
        ...
        deallocate(temp)
        ! then CQ parameters:
      num_params_function = self%PureEFTCQ%parameter_number
      allocate( temp(num_params_function) )
      do i = 1, num_params_function
          temp(i)         = array(num_params_temp)
          num_params_temp = num_params_temp +1
      end do
      call self%PureEFTCQ%init_parameters(temp)
      deallocate(temp)
        ! then gamma1:
    ...
    subroutine EFTCAMBPureEFTstdInitModelParametersFromFile( self, Ini )
        ...
        call self%PureEFTCQ%init_from_file   ( Ini )
    ...
    subroutine EFTCAMBPureEFTstdComputeParametersNumber( self )
        ...
        self%parameter_number = self%parameter_number +self%PureEFTCQ%parameter_number
    ...
    subroutine EFTCAMBPureEFTstdFeedback( self, print_params )
        ...
        if ( self%EFTCQ              /= 0 ) write(*,'(a,I3)') '   EFTCQ              =', self%EFTCQ
        ...
        call self%PureEFTCQ%feedback    ( print_params )
    ...
    subroutine EFTCAMBPureEFTstdParameterNames( self, i, name )
      ...
      integer  :: NOmega, Nw, Nq, N1, N2, N3, N4, N5, N6
      integer  :: j
      ...
      Nq     = Nw + self%PureEFTCQ%parameter_number
      N1     = Nq + self%PureEFTGamma1%parameter_number
      ...
      ! parameter from wDE function
          ...
          return
        ! parameter from CQ function
        else if ( i <= Nq ) then
            do j = 1, self%PureEFTCQ%parameter_number
                if ( i-Nw == j ) call self%PureEFTCQ%parameter_names( j, name )
            end do
            return
      ...
      ! parameter from Gamma1 function
          ...
          do j = 1, self%PureEFTGamma1%parameter_number
              if ( i-Nq == j ) call self%PureEFTGamma1%parameter_names( j, name )
          end do
          return
    ...
    subroutine EFTCAMBPureEFTstdParameterNamesLatex( self, i, latexname )
        ...
        ! parameter from wDE function
          ...
          ! parameter from CQ function
              ...
              do j = 1, self%PureEFTCQ%parameter_number
                  if ( i-Nw == j ) call self%PureEFTCQ%parameter_names_latex( j, latexname )
              end do
              return
        ...
        ! parameter from Gamma1 function
            ...
            do j = 1, self%PureEFTGamma1%parameter_number
                if ( i-Nq == j ) call self%PureEFTGamma1%parameter_names_latex( j, latexname )
            end do
            return
    ...
    subroutine EFTCAMBPureEFTstdParameterValues( self, i, value )
          ...
          ! parameter from wDE function
              ...
              return
                ...
                do j = 1, self%PureEFTCQ%parameter_number
                    if ( i-Nw == j ) call self%PureEFTCQ%parameter_value( j, value )
                end do
                return
              ...
              do j = 1, self%PureEFTGamma1%parameter_number
                  if ( i-Nq == j ) call self%PureEFTGamma1%parameter_value( j, value )
              end do
              return
    ...
    subroutine EFTCAMBPureEFTstdBackgroundEFTFunctions( self, a, eft_par_cache, eft_cache )
        ...
        select case ( self%EFTCQ )
            case(1)
              ...
              eft_cache%EFTcdot      = +0.5_dl*eft_cache%adotoa*eft_cache%grhov_t*( -3._dl*(1._dl +self%PureEFTwDE%value(a))**2 &
                  & -self%PureEFTCQ%value(a)*eft_cache%grhoc_t / eft_cache%grhov_t* (1._dl +self%PureEFTwDE%value(a))+ a*self%PureEFTwDE%first_derivative(a) ) &
              ...
              eft_cache%EFTLambdadot = -2._dl*eft_cache%EFTOmegaV*( eft_cache%Hdotdot -eft_cache%adotoa*eft_cache%Hdot -eft_cache%adotoa**3 ) &
                  ...
                  & -self%PureEFTCQ%value(a)*eft_cache%grhoc_t / eft_cache%grhov_t*self%PureEFTwDE%value(a))
    ...
    subroutine EFTCAMBPureEFTstdComputeAdotoa( self, a, eft_par_cache, eft_cache )
        ...
        case(1)
                ...
                eft_cache%grhov_t = - a**(-3._dl*self%PureEFTwDE%value(a)-1)*(- eft_par_cache%grhov*self%PureEFTCQ%value(a)-eft_par_cache%grhoc*self%PureEFTCQ%value(a)-3._dl* eft_par_cache%grhov*self%PureEFTwDE%value(a) &
                & +eft_par_cache%grhoc*self%PureEFTCQ%value(a)*a**(self%PureEFTCQ%value(a)+3._dl*self%PureEFTwDE%value(a)))/(self%PureEFTCQ%value(a)+3._dl*self%PureEFTwDE%value(a))!-------------
                eft_cache%grhoc_t = eft_par_cache%grhoc*a**(self%PureEFTCQ%value(a) -1._dl)
                eft_cache%grhom_t=eft_cache%grhob_t+eft_cache%grhoc_t+eft_cache%grhor_t+eft_cache%grhog_t
